...
function PublicApp() {
  ...
  const dailyTotals = processedDailyData.reduce((acc, day) => {
    acc.measurements += (day.measurements || 0);
    acc.portalViews += (day.portalViews || 0);
    acc.highSx += (day.highSx || 0);
    acc.orders += (day.orders || 0);
    return acc;
  }, { measurements: 0, portalViews: 0, highSx: 0, orders: 0 });
  // -----------------------------
      ...
      <div className="card" style={{
        ...
        <div className="table-container">
          <table>
            ...
            {/* Add Totals Row for Daily Data if there is data */}            
            {processedDailyData.length > 0 && (
              <tfoot>
                <tr className="totals">
                  <td className="col-day">TOTALS</td>
                  <td className="col-meas">{dailyTotals.measurements}</td>
                  <td className="col-portal">{dailyTotals.portalViews}</td>
                  <td className="col-highsx">{dailyTotals.highSx}</td>
                  <td className="col-orders">{dailyTotals.orders}</td>
                </tr>
              </tfoot>
            )}
          </table>
